...
排行榜会把原始候选结果缓存最多 60 秒。在 `CACHE_NONE` 下，该缓存无法跨请求命中，每次排行榜 API 请求都会重新执行计数、分组和排序聚合。公开站点在没有持久化主缓存时应保持排行榜关闭。每个命名用户 10 次/60 秒的 MediaWiki 标准 RateLimit 使用独立的 MicroStash；生产多节点部署必须确保所有应用实例共享一个能在完整 TTL 内可靠保存计数的 MicroStash。
